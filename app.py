...
# ═══════════════════════════════════════════════════ paso 1
st.header("1 · ¿De dónde saco los datos?")
st.caption(
    "Pega la dirección de la tienda. Puede ser la página principal o una categoría. "
    "La app detecta sola qué tecnología usa el sitio y elige la mejor forma de leer su catálogo."
)
url = st.text_input("Dirección de la tienda", placeholder="https://ejemplo.com", label_visibility="collapsed")

# ═══════════════════════════════════════════════════ paso 2
st.header("2 · ¿Qué quiero de ahí?")
st.caption(
    "Escribe en español lo que buscas. Además de recortar la lista, esto acelera muchísimo el "
    "trabajo: los productos que no te interesan se descartan antes de abrir sus fichas."
)
...
with f3:
    st.write("")
    solo_disp = st.checkbox("Solo los que están disponibles", value=False,
                            help="Descarta lo que la tienda marca como agotado.")

# ═══════════════════════════════════════════════════ paso 3
st.header("3 · ¿A dónde va el resultado?")
modo = st.radio(
    "Modo de escritura",
    MODOS,
...
# ═══════════════════════════════════════════════════ paso 4
st.header("4 · Extraer y revisar")
st.caption("Nada se escribe en tu hoja todavía. Primero te enseño lo que encontré.")

if st.button("Extraer de la tienda", type="primary"):
    if not url.strip():
        st.error("Falta la dirección de la tienda, en el paso 1.")
    elif modo == "sincronizar" and not destino.strip():
        st.error("Para sincronizar necesito la URL de tu hoja, en el paso 3.")
    else:
        limpiar_resultados()
...
if productos is not None and not productos:
